...
public class PrismProcessSuspendTest extends BaseTestClass {
    ...
    public void testSuspendSuspendedProcessOnBothColosWhen1ColoIsDown() throws Exception {
        ...
        AssertUtil.assertSucceeded(prism.getProcessHelper().suspend(bundles[0].getProcess().getName()));
        //verify
        ...
        AssertUtil.assertFailed(prism.getProcessHelper().suspend(bundles[0].getProcess().getName()));

        for (int i = 0; i < 2; i++) {
            //suspend on the other one
            AssertUtil.assertSucceeded(prism.getProcessHelper()
                .suspend(bundles[1].getProcess().getName()));
            AssertUtil
    ...
    public void testSuspendScheduledProcessOnBothColos() throws Exception {
        ...
        Assert.assertTrue(Util.parseResponse(prism.getProcessHelper()
            .getStatus(bundles[0].getProcess())).getMessage()
            .contains("SUSPENDED"));
        Assert.assertTrue(Util.parseResponse(prism.getProcessHelper()
            .getStatus(bundles[1].getProcess())).getMessage()
            .contains("RUNNING"));
    }
    ...
    public void testSuspendDeletedProcessOnBothColos() throws Exception {
        ...
        AssertUtil.assertSucceeded(prism.getProcessHelper().delete(bundles[0].getProcess().getName()));


        //suspend using prismHelper
        AssertUtil.assertFailed(prism.getProcessHelper().suspend(bundles[0].getProcess()));
        //verify
        AssertUtil.checkStatus(cluster1OC, EntityType.PROCESS, bundles[0], Job.Status.KILLED);
        AssertUtil.checkStatus(cluster2OC, EntityType.PROCESS, bundles[1], Job.Status.RUNNING);

        AssertUtil.assertSucceeded(prism.getProcessHelper().delete(bundles[1].getProcess().getName()));
        //suspend on the other one
        AssertUtil.assertFailed(prism.getProcessHelper().suspend(bundles[1].getProcess()));
        AssertUtil.checkStatus(cluster1OC, EntityType.PROCESS, bundles[0], Job.Status.KILLED);
    ...
    public void testSuspendSuspendedProcessOnBothColos() throws Exception {
        ...
        for (int i = 0; i < 2; i++) {
            //suspend using prismHelper
            AssertUtil.assertSucceeded(prism.getProcessHelper()
                .suspend(bundles[0].getProcess().getName()));
            //verify
            AssertUtil
                .checkStatus(cluster1OC, EntityType.PROCESS, bundles[0], Job.Status.SUSPENDED);
            AssertUtil.checkStatus(cluster2OC, EntityType.PROCESS, bundles[1], Job.Status.RUNNING);
    ...
    public void testSuspendNonExistentProcessOnBothColos() throws Exception {
        AssertUtil.assertFailed(prism.getProcessHelper().suspend(bundles[0].getProcess().getName()));
        AssertUtil.assertFailed(prism.getProcessHelper().suspend(bundles[1].getProcess().getName()));

        AssertUtil.assertFailed(cluster1.getProcessHelper().suspend(bundles[0].getProcess().getName()));
        AssertUtil.assertFailed(cluster2.getProcessHelper().suspend(bundles[0].getProcess().getName()));
    }
    ...
    public void testSuspendSubmittedProcessOnBothColos() throws Exception {
        ...
        AssertUtil.assertFailed(cluster1.getProcessHelper().suspend(bundles[0].getProcess().getName()));
        AssertUtil.assertFailed(cluster2.getProcessHelper().suspend(bundles[1].getProcess().getName()));
    }
    ...
    public void testSuspendScheduledProcessOnBothColosWhen1ColoIsDown() throws Exception {
        ...
        //suspend on the other one
        AssertUtil.assertSucceeded(prism.getProcessHelper().suspend(bundles[1].getProcess().getName()));
